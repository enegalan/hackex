...
class TransferController extends Controller {
    public static function checkAndUpdateTransfer (Transfer $transfer) {
        if ($transfer->status === Transfer::WORKING && now()->greaterThanOrEqualTo($transfer->expires_at)) {
            ...
            if ($transfer->type === Transfer::DOWNLOAD) ReputationActions::addReputation('download_successful', null, true, 'download_' . $transfer->id);
            elseif ($transfer->type === Transfer::UPLOAD) ReputationActions::addReputation('upload_successful', null, true, 'upload_' . $transfer->id);
